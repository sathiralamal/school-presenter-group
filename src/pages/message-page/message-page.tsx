import { IonContent, IonHeader, IonPage, IonSpinner, IonTitle, IonToolbar, IonImg } from "@ionic/react";
...
import { useComponentDidMount,
    ...
    useComponentWillUnmount } from "../../utils/ReactLifeCycle";
...
import { CACHE_USER_LOGIN_ROLE_NAME, CACHE_USER_LOGIN_ID, TENANT_ID } from "../../utils/StorageUtil";
import { v4 as uuid } from "uuid";

//redux
import { connect } from "react-redux";
import { fetchConversation, fetchMoreConversation, fetchMessages } from "../../stores/messages/actions";
...
const Message = (props: any) => {
    ...
    const [isInstantMessageActive, setIsInstantMessageActive] = useState<boolean>(false);
    const [searchLoading, setSearchLoading] = useState<boolean>(false);
    const [messageSearchLoading, setMessageSearchLoading] = useState<boolean>(false);
    ...
    let isInfiniteFirstLoad:boolean = !(!props.loading && (conversationScrollRef?.current?.beforeScrollHeight === undefined ||  conversationScrollRef?.current?.beforeScrollTop ===undefined ));
    ...
    useEffect(() => { 
        ...
        console.log("1-useEffect isFirstLoad ", isFirstLoad, " prevIsFirstLoad ", prevIsFirstLoad, " location?.state?.conversationId ", location?.state?.conversationId)
        if((isFirstLoad && isFirstLoad!==prevIsFirstLoad) && location?.state?.conversationId){
        ...
        console.log("1-useEffect props.loading ", props.loading, " prevLoading ", prevLoading)

        if(isFirstLoad || props.loading !== prevLoading){
            console.log("1-useEffect return loading changed");
            console.log("1-useEffect props.messages ", props.messages, " props.messages.length ", props.messages?.length);
            if( isFirstLoad && props.messages.length > 0){
                ...
                props.fetchMessages(state.tenantId, props.messages,0,props.messages[0].id);
            }
        ...
        if(mobile !== prevMobile){
            ...
            if( props.messages.length > 0){
                ...
                setTimeout(() => $("#ud-message-container").scrollTop($("#ud-message-container")[0]?.scrollHeight) , 1000)
        ...
        console.log("1-useEffect activeIndex ", state.activeIndex, " prevActiveIndex ", prevActiveIndex)
        if(state.activeIndex !== prevActiveIndex && props.match?.params?.id === undefined){
    ...
    },[props.loading,state.messageList, state.conversationList, 
        mobile, state.activeIndex, currentRoleName,studentOrParentContactList, 
        messageNextToken, listenerInterval])
    ...
    useEffect(() => { 
        ...
        if(lastNativeNotification !== prevLastNativeNotification){
            ...
            if(state.conversationList.length < 1){
                ...
                let conversationIndex = props.messages.findIndex((conversation:any) => conversation.id === location?.state?.conversationId);
                ...
                integration.getMessageByIdInfo(location?.state?.messageId, null).then((newMessage: any) => {
                    console.log("getMessageByIdInfo newMessage:", newMessage);
                    let conversationIndex = props.messages.findIndex((conversation:any) => conversation.id === location?.state?.conversationId);
                    console.log("getMessageByIdInfo conversationIndex:", conversationIndex);
    ...
    useEffect(() => { 
       ...
       console.log("4-useEffect props.match?.params?.id ", props.match?.params?.id, " prevParamConversationId ", prevParamConversationId);
    ...
    const  onLastNativeNotification =() => {
        ...
        console.log("onLastNativeNotification lastNativeNotification"+!lastNativeNotification+" lastNativeNotification, ...", JSON.stringify(lastNativeNotification));
        console.log("onLastNativeNotification location?.state?.data ", location?.state?.data);
        ...
        if (lastNativeNotification){
            ...
            if( state.conversationList.length === 0 && getCurrentConversation().length > 0){
                tempConversationList = getCurrentConversation();
            }
            if(lastNativeNotification?.actionId){
                conversationIndex = tempConversationList.findIndex((conversation:any) => conversation.id === lastNativeNotification.data.conversationId);
                console.log("onLastNativeNotification  lastNativeNotification?.actionId ", lastNativeNotification?.actionId, " conversationIndex ", conversationIndex);
            }
            console.log("onLastNativeNotification isString?" +  typeof(lastNativeNotification?.data));
            

            let foundMessage = false;
            if(tempConversationList?.length > 0 && conversationIndex > -1){
                foundMessage = tempConversationList[conversationIndex].messages.find((message: any) => {
            ...
            if( state?.activeIndex> -1){
                ...
                props.fetchMessages(state.tenantId, props.messages,conversationIndex,lastNativeNotification.data.conversationId);
            }else{
                // 
            ...
            if(state.activeIndex < 0){
                ...
                conversationIndex = tempConversationList.findIndex((conversation:any) => conversation.id === location?.state?.conversationId);
            }
        ...
        console.log("onLastNativeNotification step 2 newMessage ", JSON.stringify(newMessage));

        //newMessage["content"] = lastNativeNotification.body;
        console.log("onLastNativeNotification step 3 newMessage ", JSON.stringify(newMessage));

        let newMessageArray = [];
        newMessageArray.push(newMessage);
        console.log("onLastNativeNotification pushNotificationReceived newMessageArray ", JSON.stringify(newMessageArray));
    ...
    const onNewConversationOpen=(_conversationId:any) => {
        console.log("3-useEffect message-page props.match?.params ", props.match, " props.match?.params?.id ", props.match?.params?.id)
    ...
    const handleOnChangeConversation =()=>{
        ...
        if(state.activeIndex > -1 && state.conversationList?.length > 0){
            console.log("handleOnChangeConversation state.conversationList[state.activeIndex] ", state.conversationList[state.activeIndex]);
            if(state.conversationList[state.activeIndex]?.imageKey && state.conversationList[state.activeIndex]?.imageKey.length > 0){
                getProfilePhotoInfo(state.conversationList[state.activeIndex]?.imageKey, state.conversationList[state.activeIndex]?.location);
            }

            store.create().then((_storeValue:any)=>{
                store.get( state.conversationList[state.activeIndex].id).then((conversation:any)=>{
                    if(conversation !=null && conversation.isNew){
                        ...
                        handleLoadMoreMessages(state.conversationList[state.activeIndex].id, state.activeIndex);
                        store.remove( state.conversationList[state.activeIndex].id);  
    
                    }
                    console.log("2-useEffect state.activeIndex conversation ", conversation)
                })
            })
    ...
    const onMessageChange = (isNewMessageById:boolean, conversationId:any=undefined)=>{
        console.log("Enter onMessageChange state.activeIndex ", state.activeIndex, " isNewMessageById ", isNewMessageById, " conversationId ", conversationId);
        if(props?.messages?.length > 0){
            ...
            if (props.messages[state.activeIndex]?.messages) {
                console.log("onMessageChange props.messages[state.activeIndex]  : ", JSON.stringify(props.messages[state.activeIndex]));

                console.log("onMessageChange [props.messages]  props.messages[state.activeIndex]?.messages : ", JSON.stringify(props.messages[state.activeIndex]?.messages));

                console.log("onMessageChange [props.messages]  props.messages[state.activeIndex]?.messageNextToken : ", JSON.stringify(props.messages[state.activeIndex]?.messageNextToken));
                newState.messageList = [...props.messages[state.activeIndex]?.messages];
                newState.messageNextToken = props.messages[state.activeIndex]?.messageNextToken;
            }
            
            if(isNewMessageById && conversationId){
                let conversationIndex = props.messages.findIndex((conversation:any) => conversation.id === conversationId);
                newState.activeIndex = conversationIndex;
                console.log("onMessageChange newState.activeIndex ", newState.activeIndex); 
                setState({ ...state, activeIndex:conversationIndex })
            }
            setState({...state, ...newState} )
    ...
    useComponentDidMount(() => {
      ...
      const manageConversationListener=()=>{
        console.log("manageConversationListener props?.messages?.length ", props?.messages?.length);
            ...
            store.create().then((storeValue:any)=>{
                ...
                store.get("LISTENED_CONVERSATION").then((_lisConversations:any)=>{
                    ...
                    if(lisConversations === null){
                        ...
                        let lisIndex = lisConversations.findIndex((conver: any) =>conver.conversationId === props?.conversation?.id);
                        if( lisIndex > -1){
                            ...
                            if(Object.keys(listeners).length > 0){
                                ...
                                store.set("LISTENED_CONVERSATION", JSON.stringify([ ...lisConversations, {conversationId:state.conversationList[state.activeIndex]?.id, timestamp:new Date(), listeners}]) );
                                console.log("ConversationListener: Added listener of conversation Id ", state.conversationList[state.activeIndex]?.id);
                            }
    ...
    const handleListenMessageChange = ()=>{
        ...
        try {
            ...
            if(state.conversationList[state.activeIndex]?.id && state.conversationList[state.activeIndex]?.id!==undefined ){
                let conversationListener = integration.onMessageByConversationIdInfo(state.conversationList[state.activeIndex]?.id, (value: any) => {
                
                console.log("onMessageByConversationIdInfo value.message ", value.message);
                try{
                    //console.log("onMessageByConversationIdInfo props.conversation ", props.conversation);
                    if(value.message.messageLevel ==="reply" || value.message.conversationType === "member_to_group" ){
                        console.log("onMessageByConversationIdInfo message is a reply can't process ");

                        return;
                    }
                    let activeConversation = state.conversationList[state.activeIndex];
                    let foundMessage;
                    let msgIndex = -1;
                    if (activeConversation?.messages) {
                        ...
                        msgIndex = activeConversation.messages.findIndex((message: any) => message.id === value.message.id);
                    ...
                    if( !foundMessage ){
                        ...
                        if(value.message.stage === "queued" && userLoginId !== value.message.createdByUserId){
                            //console.log("onMessageByConversationIdInfo send open event");
                            integration.createOpenMessageEventInfo(uuid(), value.message.id, "anyone").then((value:any)=>{
                                        integration.updateMessageStageOpen(value.message.id).then((retValue:any)=>{});
                            })  
                        }
                        msgIndex = activeConversation.messages.length;
                        toMessageObjectUI(value.message, null, isBroadcast,null).then((value) => {
                    ...
                    console.error("onMessageByConversationIdInfo 1 errorMsg" , errorMsg);
                    if(errorMsg?.error?.errors?.length > 0 && errorMsg?.error?.errors[0].message==="Connection closed"){
                        console.log("onMessageByConversationIdInfo clear all LISTENED_CONVERSATION");
                ...
                let sessionListener = integration.onSessionEventByUserIdInfo(props.userLogonId, (value: any) => {
                    //console.log("onSessionEventByUserId value ", value);
                });
                retVal = {
                    conversationListener,sessionListener}
            }else{
            ...
            console.log("handleListenMessageChange onMessageByConversationIdInfo error ", error);
            try{
                console.log("handleListenMessageChange stopped listening ", props.conversation.id); 
                store.create().then((storeValue:any)=>{
                store.remove(props.conversation.id).then((lisConversations:any)=>{});
                });
            }catch(errorRemove){
    ...
    const createInterval = () => {
        ...
        let newInterval =  setInterval(
            async() => {
                    ...
                    console.log("Listener running Internal window.location ", window?.location);
                    ...
                    if(props?.messages?.length > 0 && props.messages[state.activeIndex]?.id && props.messages[state.activeIndex]?.id!==undefined){
                        console.log("invoked manageConversationListener");
                        manageConversationListener();
                    }else{
                        console.log("cant invoke manageConversationListener due to props?.messages?.length ", props?.messages?.length ," state.activeIndex ",state.activeIndex );          
                    }
                }, 120000
        )
    ...
    const updateMessages = async() => {
        ...
        store.forEach((value:any, key:any, index:any) => {
            ...
            if (key.indexOf("MESSAGE_") > -1) {
                ...
                integration.getMessageByIdInfo(messageId, null).then((newMessage: any) => {
    ...
    const updateConversation = (newMessage: any, conversationIndex:any) => {
        ...
        console.log("updateConversation state.conversationList :", state.conversationList);
        console.log("updateConversation conversationList :", state.conversationList.length);
        console.log("updateConversation getCurrentConversation :", getCurrentConversation());
        console.log("updateConversation getCurrentConversation.length :", getCurrentConversation().length);

        console.log("updateConversation props.messages length:", props.messages.length);
        ...
        console.log("updateConversation latestConversationList :", latestConversationList);

        for (let index = 0; index < latestConversationList.length; index++) {
            let foundMessage;
            console.log("focus latestConversationList[index] :", latestConversationList[index]);
            if (latestConversationList[index].id === newMessage[0].conversationId) {
                isConversationFound = true;
                foundMessage = latestConversationList[index].messages.find((message: any) => {
                   
                    return message.id === newMessage[0].id;
                });
                if (!foundMessage) {                
                    toMessageObjectUI(newMessage[0], null, newMessage[0]?.isGroupConversation, null).then((value: any) => {
                        console.log("toMessageObjectUI value ", JSON.stringify(value) );
                        console.log("toMessageObjectUI latestConversationListindex].messages before ", latestConversationList[index].messages );
                        latestConversationList[index].messages.push(value);

                        console.log("toMessageObjectUI latestConversationList[index].messages after ", latestConversationList[index].messages );
    
                        onConversationListChange(latestConversationList.slice(), conversationIndex);
                    });
                }
            }        
        }
        if(!isConversationFound){
            console.log("No pre-existing conversations. A specific conversation will be collected");

            let promiseConversationAndMessage = [];

            promiseConversationAndMessage.push(integration.listChatConversationByIdInfo(newMessage[0].conversationId))
            ...
            Promise.all( promiseConversationAndMessage).then(conversationAndMessage=>{
                let conversationModel:any = {};
                conversationModel["items"] = conversationAndMessage[0];
                
                toConversationUI(conversationModel).then((uiConversationModel:any)=>{
                    console.log("toConversationUI uiConversationModel ", uiConversationModel);
                    toMessageObjectUI(newMessage[0], null, false, null).then((value: any) => {
                        ...
                        if(state.conversationList.length > 0 ){
                            newConversationList = [...state.conversationList , ...uiConversationModel];
                        }else{
    ...
    const onConversationListChange = (updatedConversations: any, _conversationIndex:any ) => {
        console.log("onConversationListChange updatedConversations ", updatedConversations , " _conversationIndex ", _conversationIndex);
        console.log("onConversationListChange updatedConversations  state.activeIndex ",state.activeIndex); 
        let conversationIndex = state.activeIndex;
        

        if(state.activeIndex < 0 && lastNativeNotification){
            conversationIndex = state.conversationList.findIndex((conversation:any) => conversation.id === lastNativeNotification.data.conversationId);
            updatedConversations[conversationIndex] = { ...updatedConversations[conversationIndex] , unread: updatedConversations[conversationIndex]?.unread + 1};
        } 
    ...
    const deleteMessageInConversation = (conversationId: any, messageId: any) => {
        let conversationIndex = state.conversationList.findIndex((conversation:any) => conversation.id === conversationId);
        let conversation = state.conversationList[conversationIndex];
        let messageIndex = conversation.messages.findIndex((message: any) => message.id === messageId);
        conversation.messages.splice(messageIndex, 1);
        let values = state.conversationList.slice();
        setState({ ...state, conversationList:values , messageList: values[state.activeIndex]?.messages })
    };
    ...
    const getClassDetails = (role: any, classData: any, roleIndex: number, gradeIndex: number, classIndex: number) => {
        integration.listClassNamesByGradeIdInfo(classData.id, null).then((res: any) => {
            if (res?.items) {
    ...
    const getClassList = (
        ...
        integration.getUserByRoleAndClassInfo(roleId, classId, null).then((res: any) => {
            if (res?.items) {
                res?.items.forEach((g: any) => (g.roleName = g.className));
                cascadeRawData[roleIndex].children[gradeIndex].children[classIndex].children = res?.items;
                setCascadeData(cascadeRawData);
    ...
    const searchMessages = (text: string) => {
        if (text.length >= 3) {
            ...
            setState({ ...state, messageList:[...props.messages[state.activeIndex]?.messages] })
        }
    };
    const handleOpenConversation = (index: number, isResetMessageNextToken:boolean) => {
        console.log("Enter handleOpenConversation index ",index, " isResetMessageNextToken ", isResetMessageNextToken)
        
        let newState = {
            activeIndex : index,
            messageList: getCurrentConversation()[index]?.messages,
            messageNextToken:!isResetMessageNextToken? undefined:getCurrentConversation()[index]?.messageNextToken,
        ...
        console.log("----handleOpenConversation getCurrentConversation()[index]?.messages ", getCurrentConversation()[index]?.messages )
        console.log("----handleOpenConversation getCurrentConversation() ", getCurrentConversation() )
        console.log("----handleOpenConversation getCurrentConversation()[index]?.id ", getCurrentConversation()[index]?.id )
    ...
    const handlePagination = async () => {
        try {
            ...
            await props.fetchMoreConversation(
                nextToken
            );
        } catch (err) {
        } finally {
    ...
    const handleLoadMoreMessages = async (conversationId: string, activeIndex:any) => {
        console.log("[message-page.tsx] handleLoadMoreMessages conversationId ", conversationId, 
        " messageNextToken ", messageNextToken, "activeIndex ", activeIndex);
        let currentConversationList = getCurrentConversation();
        currentConversationList[activeIndex].messageNextToken = undefined;

        props.fetchMessages(state.tenantId, state.conversationList?.length > 0? state.conversationList:currentConversationList ,activeIndex,conversationId);
    ...
    const handleHasMoreMessages = ()=>{
        console.log(">>>>>handleHasMoreMessages return ", messageNextToken === undefined || messageNextToken!=null);
        console.log(">>>>>handleHasMoreMessages lastNativeNotification ", messageNextToken === undefined || messageNextToken!=null);
        ...
        <>  
            ...
            <Grid fluid className="message-page">
                <Row className="show-grid">
                    {(!mobile || (mobile && !state.conversationList[state.activeIndex])) && (
                        <Col lg={8} md={8} xs={24}>
                            <div className="search-header">
                                {mobile && currentRoleName !== "Student" && currentRoleName !== "Parent" && (
                                    <CreateConversation />
                                ...
                                {mobile && (currentRoleName === "Student" || currentRoleName === "Parent") && (
                                    ...
                                    <Dropdown color="green" appearance="ghost" title="+ New Message" onChange={()=>console.log("clicked + New Message")}>
                                        {studentOrParentContactList.map((contact: any, i: number) => {
                                            return (
                                                <Dropdown.Item
                                                    key={i}
                                                    onClick={() => handleCreateConversation(contact.id , contact )}
                                                >
                                                    <IonImg
                                                        src={
                                ...
                                <InputGroup className="search-input-message">
                                    <Input
                                        onKeyUp={(event: any) => searchChatConversationsInfo(event.target.value)}
                                        placeholder="Search"
                                    />
                                    <InputGroup.Addon>
                                        {searchLoading ? (
                            ...
                            {props.loading && (
                                <p style={{ color: "#28ba62", marginBottom: 10 }}>Fetching messages...</p>
                            )}
                            {/* <InfiniteScroll
                                    pageStart={0}
                                ...
                                > */}
                            <div id="conversationDiv" className="conversation-list my-scrollbar">
                            <InfiniteScroll 
                                        initialLoad = {false}
                                        pageStart={0}
                                        loadMore={async() => {
                                            console.log("contactsTable loadMore data ... isFirstLoad ", isFirstLoad ," userLoginId ", userLoginId );
                                            if(isFirstLoad){
                                        ...
                                        loader={
                                            <>
                                                {isInfiniteFirstLoad && <div style={{ textAlign: "center", padding: 10 }}>
                                                <IonSpinner
                                                    name="bubbles"
                                                    style={{ transform: "scale(1.5)" }}
                                                    color="success"
                                    ...
                                    >
                                {state.conversationList.map((conversation:any, index:number) => (
                                    conversation?.id? <Conversation
                                        index={index}
                                        key={conversation.id}
                                        openConversation={(index: number) => handleOpenConversation(index, false)}
                                        active={index == state.activeIndex}
                                        conversation={conversation}
                                        conversationList={state.conversationList}
                                        onConversationListChange={onConversationListChange}
                                        deleteMessageInConversation={deleteMessageInConversation}
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        <IonImg
                                                            ...
                                                            className="groupAdminsIcon"
                                                            style={{ width: "15%", margin: "0 12px", verticalAlign: "middle" }}
                                                        />
                                                        {contact.firstName} {contact.lastName}{" "}
                                                        <span className="text-success">
                                                            <b>
                                    ...
                                    {state.conversationList[state.activeIndex] && (
                                            ...
                                            <div className="header-block">
                                                ...
                                                <div className="user-detail">
                                                    <Avatar
                                                        ...
                                                        src={avatar}
                                                        size="md"
                                                        alt={state.conversationList[state.activeIndex]?.name
                                                            ?.toString()
                                                            ?.substr(0, 1)}
                                                    >
                                                    ...
                                                    <h2 className="name">{state.conversationList[state.activeIndex].name}</h2>
                                                </div>
                                                {!mobile && (
                                                    ...
                                                    >
                                                        <Input
                                                            onKeyUp={(event: any) => searchMessages(event.target.value)}
                                                            placeholder="Search"
                                                        />
                                                        <InputGroup.Addon>
                                        ...
                                        >
                                                <div className="message-list clearfix">
                                                    ...
                                                    {state.messageList?.map((message: any, index: number) => (
                                                        Object.keys(message)?
                                                        <MessageComponent
                                                            conversation={state.conversationList[state.activeIndex]}
                                                            conversationList={state.conversationList}
                                                            ...
                                                            activeIndex={state.activeIndex}
                                                            conversationId={state.conversationList[state.activeIndex].id}
                                                            index={index}
                                                            setReplyNotificationView={setReplyNotificationView}
                                                            setReplyMessageId={setReplyMessageId}
                                                            replyMessageId={replyMessageId}
                                                            deleteMessage={(conversationId: any, messageId: any) => {
                                                                console.log(
                                                                ...
                                                                deleteMessageInConversation(conversationId, messageId);
                                                            }}
                                                        />:"No Message"
                                            ...
                                            {( state.conversationList[state.activeIndex]?.isBroadcast && state.conversationList[state.activeIndex]?.isGroupAdminUser) &&
                                            <ChatControls
                                                onConversationListChange={onConversationListChange}
                                                conversationList={state.conversationList}
                                                conversation={state.conversationList[state.activeIndex]}
                                                ...
                                                setSelectedMessage={setSelectedMessage}
                                                onUpdateMessage={(messageId: string, newMessage: string) => {
                                                    console.log(newMessage);
                                                    updateMessageContent(messageId, newMessage);
                                                }}
                                            />}
                                             {( !state.conversationList[state.activeIndex]?.isBroadcast) &&
                                            <ChatControls
                                                onConversationListChange={onConversationListChange}
                                                conversationList={state.conversationList}
                                                conversation={state.conversationList[state.activeIndex]}
